...
this.IDforRecord()+" and classname: "+this.classNameForRecord())},checkCookies:function(){var a="current_section_heading_"+this.uniquePageID();if(a=FrontEndEditForm.getCookie(a))FrontEndEditForm.formObject.find("h3.opened").click(),0<jQuery("h3#"+a).length&&jQuery("h3#"+a).click()},dontTabOnAlsoEdit:function(){jQuery(".modalPopUp").attr("tabIndex",-1)},getCookie:function(a){a+="=";for(var b=document.cookie.split(";"),c=0;c<b.length;c++){for(var d=b[c];" "==d.charAt(0);)d=d.substring(1);if(0==d.indexOf(a))return d.substring(a.length,
d.length)}return""},openMySection:function(){if(jQuery(this).is(":visible"))console.debug("visible");else{console.debug("not visible");var a=jQuery(this).prev("h3:first");jQuery(a).hasClass("closed")&&jQuery(a).click()}},initRemoveLink:function(){jQuery("body").on("click",FrontEndEditForm.deleteLinkSelectors,function(a){a.preventDefault();var b=this;a=jQuery(b).attr("href");jQuery(b).parent().addClass("removingNow").fadeOut();jQuery.get(a,function(){var a=jQuery(b).closest(".frontendextendedhasone").find("select");
console.debug(a);jQuery(b).parent().hide();jQuery(b).removeClass("removingNow");a.length&&(console.debug("test"),a.val(0))});return!1})},checkHowCompleteFormIs:function(a){var b=0,c=0,d=!1;FrontEndEditForm.formObject.find("div.field").each(function(e,l){if(!jQuery(this).hasClass("readonly")){var g=jQuery(l),f=jQuery(l).find("input, textarea, select"),k=f.attr("id"),h=g.prevAll("h3:first").text(),m=g.prevAll("h3:first").attr("id")+"_NeedsCompletion";f.hasClass("checkbox")?(g=g.find("label").text(),
f.attr("checked")?(jQuery("#"+k+"NeedsCompletion").remove(),d=!1,c++):d=!0):(g=g.find("label.left").text(),f=f.val(),"undefined"!=typeof f&&(0<f.length&&"0"!==f&&"0.00"!==f?(jQuery("#"+k+"NeedsCompletion").remove(),d=!1,c++):d=!0));d&&a&&(0==jQuery("#"+m).length&&jQuery("#FieldsToCompleteList").append('<dt id="'+m+'">'+h+"</dt>"),jQuery("#FieldsToCompleteList").append('<dd id="'+k+'NeedsCompletion">'+g+"</dd>"));b++}});var e=Math.round(c/b*100);0==e?jQuery("#ProgressBar").hide():(jQuery("#ProgressBar").show(),
jQuery("#CompletionPercentage").animate({width:e+"%"},500))},progressBarListeners:function(){jQuery("#ProgressBar").on("click",function(){0==jQuery("#FieldsToCompleteList dd").length&&FrontEndEditForm.checkHowCompleteFormIs(!0);jQuery("#FieldsToCompleteList").slideToggle(400,function(){})});jQuery(document).on("click","#FieldsToCompleteList dt",function(){var a=jQuery(this).attr("id").replace("_NeedsCompletion","");jQuery("#"+a).trigger("click")})},deleteConfirmation:function(){jQuery("input[name='action_deleterecord']").on("click",
function(a){confirm("Are you sure you want to delete this record?")||a.preventDefault()})},ajaxValidation:function(){FrontEndEditForm.formObject.on("change blur","div.field.ajax-validation",function(){this.classNameForRecord();this.isNewRecord()})},_uniquePageID:"",uniquePageID:function(){""===this._uniquePageID&&(this._uniquePageID=this.classNameForRecord()+"_"+this.IDforRecord());return this._uniquePageID},_IDforRecord:0,IDforRecord:function(){0===this._IDforRecord&&(this._IDforRecord=jQuery("input[name='IDToUse']").attr("value"));
return this._IDforRecord},_classNameForRecord:0,classNameForRecord:function(){0===this._classNameForRecord&&(this._classNameForRecord=jQuery("input[name='ClassNameToUse']").attr("value"));return this._classNameForRecord},isNewRecord:function(){return 0==this.IDforRecord()?!0:!1}};
